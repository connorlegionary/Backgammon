...
//    property var game: syncManager.gamesDatabase.getGame(viewContext.id)
    ...
    ComputerGamesDatabase {
        ...
        Component.onCompleted: {
            computerModel.setGameId(viewContext.id)
            computerModel.setMatchId(viewContext.id)
            board.showStartGame = !computerModel.loadMoves(viewContext.id)
            if (!board.showStartGame)
            {
                if (computerModel.lastTurnColor(viewContext.id))
                {

                    computerModel.userFinishedTurn(false);
                } else {
                    computerModel.computerTurnFinish(false);
                }
            }

            console.log("showStartGame: " + board.showStartGame)
        }
    }


    property bool computerNextTurn: false
    Timer {
        interval: 10
        running: true
        repeat: false
        onTriggered: {
            computerModel.positionsChanged()
        }
    }
...
//            doubleText.text: game.doubleVal
    ...
    ComputerPieceModel {
        ...
        onNoMoves: {
            computerNextTurn = isComputer
            nextTurnTimer.start()
        }
        onWonGame: {
//            isWhite, current
            gameDb.finishGame(viewContext.id)
            if (isWhite)
            {
                // user won
                pushOverlay("qrc:/Overlays/WonMatchComputer.qml", { white: isWhite })
            } else {
                // computer won
                pushOverlay("qrc:/Overlays/LostMatchComputer.qml", {white: isWhite })
            }
...
//            }
    ...
    Timer {
        ...
        repeat: false
        onTriggered: {
            console.log("Turn timer trigger")
            if (computerNextTurn)
            {
                computerModel.computerTurnFinish()
            } else {
                computerModel.userFinishedTurn()
            }
        }
    }
    ...
    Item {
        ...
        Rectangle {
            ...
            MouseArea
            {
                anchors.fill: parent
                onClicked: {
                    board.model.startComputerMoves();
                }
            }
        }
        ...
        Image {
            ...
            MouseArea
            {
                anchors.fill: parent
                onClicked: {
                    pushOverlay("qrc:/Overlays/Analysis.qml", {pieces: board.model.pieces, color: board.model.isWhite, diceRoll: board.model.getLastRoll() })
                }
            }
        }
        ...
        Image {
            ...
            MouseArea
            {
                anchors.fill: parent
                onClicked: {
                    pushOverlay("qrc:/Overlays/Playback.qml", {id: viewContext.id, pieces: board.model.pieces, color: board.model.isWhite, diceRoll: board.model.getLastRoll(), model: board.model })
                }
            }
        }
